...
There are two ways MetaSync signs in as that user, chosen as **Connection method** on step 1 of the connect dialog, and the boundary is the same either way. **Server-to-server** (recommended) sends the app's key and secret to the org's My Domain URL and receives a token minted as the **Run As** user named on the app's Client Credentials Flow policy: no login page, no person in the loop, and no refresh token. **Sign in as a user** has someone sign in once on a Salesforce login page as the integration user; MetaSync keeps the refresh token that sign-in produces, and the token belongs to whoever authorised. Whichever you pick, point it at the same locked-down user described below.
...
| **Enable Client Credentials Flow** | On | Server-to-server | Under the app's OAuth settings. It is what lets MetaSync exchange the app's key and secret for an access token, with no login page. Treat the pair as the credential it is: **anyone holding the key and secret can mint tokens while the flow is enabled**, at the Run As user's permission level. |
| **Policies → Client Credentials Flow → Run As** | The integration user | Server-to-server | Every token this flow mints acts as this user, so the permission set on it is the entire boundary. An API-only **Integration User** licence works here. Note that the *Permitted Users* policy does **not** apply to the Run As user — this setting is what bounds the flow instead. |
| **Require PKCE** | On | Sign in as a user | MetaSync uses the authorization-code flow with PKCE (S256) and a random `state` value. |
| **Require secret for Web Server Flow** | On | Sign in as a user | MetaSync holds the consumer secret and sends it. |
| **Require secret for Refresh Token Flow** | On | Sign in as a user | Same — the refresh exchange is authenticated. |
| **Refresh Token Rotation** | On or off | Sign in as a user | Either setting works. MetaSync stores each rotated token as Salesforce issues it and re-reads the newest stored token before every refresh, so a rotating org and a non-rotating one both keep working. If your security standard requires rotation, turn it on. |
| **Permitted Users** | *Admin approved users are pre-authorized* | Sign in as a user | With *All users may self-authorize*, anyone in the org holding the Consumer Key and Secret can authorise this app as **themselves** and receive an `api`-scoped token at their own permission level — at which point the integration user has stopped being the boundary. Pre-authorize the integration user's profile or permission set so only that user can mint a token for this app. [Connected Apps OAuth Usage](#security-monitoring) shows who has actually authorised it. |
...
> **Note — Server-to-server uses your My Domain URL.** The token request for the Client Credentials Flow goes to the org's **own** address — `https://<name>.my.salesforce.com`, copied from **Setup → My Domain**. Salesforce does not accept `login.salesforce.com` or `test.salesforce.com` for this flow, and MetaSync refuses them on step 1 with the reason under the field. The sign-in method accepts all three.

> **Note — Revocation is yours.** Because the app lives in your org, you end the connection there, and MetaSync cannot re-establish access on its own. **Server-to-server:** rotate the Consumer Secret, untick **Enable Client Credentials Flow**, or deactivate the Run As user. Rotating the secret or deactivating the user cuts access at MetaSync's very next token request. Unticking the flow stops **new** tokens but does not revoke tokens Salesforce has already issued — MetaSync's cached access token lives at most 90 minutes, which is the outside window before access stops. **Sign in as a user:** remove the app's authorisation for the integration user, deactivate the user, or delete the app; the next sync fails to authenticate and stops. Uninstalling the Confluence app purges the stored credentials from Forge storage either way; see [Permissions, scopes & data handling](troubleshooting.md#ts-permissions-scopes).
