...
MetaSync signs in as a **dedicated integration user** in your org. The refresh token belongs to whoever authorises the connection, and that user's permissions define everything MetaSync can see — so this user, and not the OAuth scope, is where you set the boundary. Create it for MetaSync alone and give it nothing else to do.
...
*The settings on that app that carry security weight, and what MetaSync needs each to be.*

| Setting | Value | Why |
| --- | --- | --- |
| **OAuth scopes** | `api` and `refresh_token` / `offline_access` — nothing else | Salesforce has no metadata-only scope, so `api` is the narrowest that permits Metadata and Tooling API calls. What it can reach is bounded by the integration user, not by this. |
| **Require PKCE** | On | MetaSync uses the authorization-code flow with PKCE (S256) and a random `state` value. |
| **Require secret for Web Server Flow** | On | MetaSync holds the consumer secret and sends it. |
| **Require secret for Refresh Token Flow** | On | Same — the refresh exchange is authenticated. |
| **Refresh Token Rotation** | **Off** | MetaSync keeps one long-lived refresh token. With rotation on, the connection breaks after the first sync. |
| **Permitted Users** | *Admin approved users are pre-authorized* | With *All users may self-authorize*, anyone in the org holding the Consumer Key and Secret can authorise this app as **themselves** and receive an `api`-scoped token at their own permission level — at which point the integration user has stopped being the boundary. Pre-authorize the integration user's profile or permission set so only that user can mint a token for this app. [Connected Apps OAuth Usage](#security-monitoring) shows who has actually authorised it. |
| **IP Relaxation** | Relax, if you enforce login IP ranges | MetaSync's calls originate from Atlassian's cloud, whose addresses vary. See also the identity-verification note below. |

> **Note — Revocation is yours, and it is immediate.** Because the app lives in your org, you end the connection there: remove its authorisation for the integration user, deactivate the user, or delete the app. MetaSync's next sync fails to authenticate and stops — it cannot re-establish access on its own, because the refresh token is the only credential it holds. Uninstalling the Confluence app purges that token from Forge storage; see [Permissions, scopes & data handling](troubleshooting.md#ts-permissions-scopes).
